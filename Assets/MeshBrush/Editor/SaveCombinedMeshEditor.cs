...
namespace MeshBrush {
    ...
    public class SaveCombinedMeshEditor : Editor {
        ...
        {
            SaveCombinedMesh _mbs = (SaveCombinedMesh)target;
            MeshFilter meshFilter = _mbs.GetComponent<MeshFilter>();
            EditorGUILayout.Space();

            if (!meshFilter && Event.current.type == EventType.Repaint)
            {
                DestroyImmediate(_mbs);
                return;
            }
