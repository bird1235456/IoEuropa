...
namespace MeshBrush
{
    ...
    public class MeshBrushEditor : Editor
    {
        ...
        GUIContent toolTipColor, toolTipRadius, toolTipFreq, toolTipOffset, toolTipSlopeInfluence, toolTipSlopeFilter, toolTipInvSlope, toolTipManualRefVecS, toolTipRefVecSample, toolTipTangentY, toolTipInvertY, toolTipInset, toolTipNR, toolTipUniformly, toolTipUniformlyRange, //Tooltips for the various gui elements.
            toolTipWithinRange, toolTipRot, toolTipV4, toolTipReset, toolTipAddScale, toolTipFlagS, toolTipCombine, toolTipDelete, toolTipPrecisionPlacementMode;
        ...
        public override void OnInspectorGUI() // Works like OnGUI, except that it updates only the inspector view.
        {
            ...
            if (help)
            {
                EditorGUILayout.HelpBox("Paint meshes onto your GameObject's surface.\n_______\n\nKeyBoard shortcuts:\n\nPaint meshes:\tpress or hold    " + _mp.paintKey + "\nDelete meshes:\tpress or hold    " + _mp.deleteKey + "\nCombine meshes:\tpress or hold    " + _mp.combineAreaKey + "\nIncrease radius:\tpress or hold    " + _mp.increaseRadiusKey + "\nDecrease radius:\tpress or hold    " + _mp.decreaseRadiusKey + "\n_______\n\n" +
                    "Assign one or more prefab objects to the 'Set of meshes to paint' array below and press " + _mp.paintKey + " while hovering your mouse above your GameObject to start painting meshes. Press " + _mp.deleteKey + " to delete painted meshes." +
                    ...
                    "Check out the documentation PDF that comes with MeshBrush (or the YouTube tutorials) to find out more about the individual parameters and functions (but most of them should be quite self explainatory, or at least supplied with a tooltip text label after hovering your mouse over them for a couple of seconds).\n_______\n\n" +
            ...
            if (_mp.paintBuffer.Count < bufferSize)
            {
                if (nextBufferRefill < Time2Die || nextBufferRefill == 0d)
                {
                    ...
                    for (int i = 0; i < bufferSize / 6; i++)
                    {
                        ...
                        GameObject bufferedObj = (GameObject)PrefabUtility.InstantiatePrefab(_mp.setOfMeshesToPaint[r]);
        ...
        void BrushMode_PrecisionPlacement() // Mode for the precise placement of single meshes.
        {
            if (Selection.gameObjects.Length == 1 && Selection.activeGameObject.transform == thisTransform)
            {
                ...
                if (thisCollider.Raycast(scRay, out scHit, Mathf.Infinity))
                {
                    ...
                    if (_mp.setOfMeshesToPaint.Length > 1)
                    {
                        if (Event.current.type == EventType.KeyDown && _phase == 0)
                        ...
                        if (_i < 0) _i = _mp.setOfMeshesToPaint.Length - 1;
                        if (_i >= _mp.setOfMeshesToPaint.Length) _i = 0;
                    }
                    else _i = 0;
                    _i = Mathf.Clamp(_i, 0, _mp.setOfMeshesToPaint.Length - 1);
                }
                ...
                switch (_phase)
                {
                    case 0:
                        ...
                        {
                            previewObj.transform.position = scHit.point;
                            Handles.Label(scHit.point + Vector3.right + Vector3.up, "Current object: " + _mp.setOfMeshesToPaint[_i].name + "\t\t\t1/4\n\nChoose a location for your object\n\nSelect next: [N]\nSelect previous: [B]\n\nConfirm location: [" + _mp.paintKey + "] / Cancel placement: [ESC]", EditorStyles.helpBox);
                        }
                    ...
                    case 1:
                        ...
                        if (previewObj != null)
                        {
                            ...
                            Handles.Label(scHit.point + Vector3.right + Vector3.up, "Current object: " + _mp.setOfMeshesToPaint[_i].name + "\t\t\t2/4\n\nAdjust the scale of your object by dragging\nthe mouse from/to the object's center\n\nConfirm scale: [" + _mp.paintKey + "] / Cancel placement: [ESC]", EditorStyles.helpBox);
                        }
                    ...
                    case 2:
                        ...
                        if (previewObj != null)
                        {
                            ...
                            Handles.Label(scHit.point + Vector3.right + Vector3.up, "Current object: " + _mp.setOfMeshesToPaint[_i].name + "\t\t\t3/4\n\nAdjust the rotation along the Y-Axis by dragging\nyour cursor horizontally\n\nConfirm rotation: [" + _mp.paintKey + "] / Cancel placement: [ESC]", EditorStyles.helpBox);
                        }
                    ...
                    case 3:
                        ...
                        if (previewObj != null)
                        {
                            ...
                            Handles.Label(scHit.point + Vector3.right + Vector3.up, "Current object: " + _mp.setOfMeshesToPaint[_i].name + "\t\t\t4/4\n\nAdjust the offset along the Y-axis by dragging\nyour cursor vertically\n\nConfirm offset: [" + _mp.paintKey + "] / Cancel placement: [ESC]", EditorStyles.helpBox);
                        }
                ...
                // Here I give the user the chance to cancel the precision placement mode by pressing the escape key.
